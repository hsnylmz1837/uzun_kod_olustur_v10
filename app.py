...
# ---- Titles (v10) ----
st.set_page_config(page_title="Uzun Kod — v10 / Statik", page_icon="🧩", layout="wide")
header = st.container()
with header:
    left, right = st.columns([6,1])
    with left:
        st.title("Uzun Kod Oluşturma Programı - v10 / Statik")
...
def pad_number(n, pad):
    # Robustly treat missing/NaN/blank pad as "no padding"
    if pad is None or (isinstance(pad, float) and math.isnan(pad)) or (isinstance(pad, str) and pad.strip() == ""):
        try:
            nf = float(n)
            return str(int(nf)) if nf.is_integer() else str(nf).rstrip("0").rstrip(".")
        except Exception:
            return str(n)
    # If pad given as integer-like (e.g., 4 or "4") -> zero-pad to that width
    if isinstance(pad, (int, float)) and not math.isnan(pad):
        width = int(pad)
        nf = float(n)
        return f"{int(nf):0{width}d}" if nf.is_integer() else f"{nf:0{width}.0f}"
    if isinstance(pad, str) and pad.isdigit():
        width = int(pad)
        nf = float(n)
        return f"{int(nf):0{width}d}" if nf.is_integer() else f"{nf:0{width}.0f}"
    # If pad like "6.2" -> fixed width with decimals, drop dot in output
...
def prereq_met(prereq_key_raw)->bool:
    prereq_key = normalize_prereq(prereq_key_raw)
    if not prereq_key: return True
        ...
        if secs.empty: st.warning("Bu makine için 'sections' sayfasında kayıt yok.")
            ...
            for i, (_, sec) in enumerate(secs.iterrows()):
                with tabs[i]:
                    ...
                    for _, fld in fields.iterrows():
                        k = fld["FieldKey"]; label = fld["FieldLabel"]; typ = str(fld["Type"]).lower(); req = bool(fld["Required"]); default = fld.get("Default"); prereq_raw = fld.get("PrereqFieldKey"); enabled = prereq_met(prereq_raw)
                        prereq_key = normalize_prereq(prereq_raw)
                        if not enabled and prereq_key:
